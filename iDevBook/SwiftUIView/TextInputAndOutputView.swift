...
extension View {
    /// Applies a different transform based on the provided value.
    /// - Parameters:
    ///   - value: The value to evaluate.
    ///   - cases: A dictionary mapping values to transformations.
    ///   - defaultTransform: A fallback transformation if the value doesn't match any case.
    /// - Returns: A modified `View` based on the matching case.
    @ViewBuilder func `switch`<V: Hashable, Content: View>(
        _ value: V,
        cases: [V: (Self) -> Content],
        default defaultTransform: ((Self) -> Content)? = nil
    ) -> some View {
        if let transform = cases[value] {
            transform(self)
        } else if let defaultTransform = defaultTransform {
            defaultTransform(self)
        } else {
            self
        }
    }
                ...
                List {
                    ...
                    Section("Control") {
                        ...
                        if selectedTextInputAndOutputType == .textField {
                            ...
                            //                                    selection:
                            //                                        $selectedTextInputAutocapitalization
                            //                                ) {
                            //                                    ForEach(
